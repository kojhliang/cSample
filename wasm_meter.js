...
Module['postRun'] = [];
  ...
  var wasmBinaryFile = `${process.argv[2]}`;
  var wasmMeterBinaryFile = `test_meter.wasm`;
  var asmjsCodeFile = 'test.temp.asm.js';

//Add wasm_meters begin -- by lmh 20181019
const fs = require('fs')
const metering = require('../meters/')
const gasPrice=1 //Assumes the gas priceGas is 1wei
const limit = 1000000000 * gasPrice // limit 1eth=10^9 wei
let gasUsed = 0;
 //end
 
 //Wasm meters begin --lmh 20181016
function injectWasmFile(){
  console.log("1.Preapre Execute inject meters code into wasm ...")  
  const wasm = fs.readFileSync(wasmBinaryFile)
  const meteredWasm = metering.meterWASM(wasm, {
     meterType: 'i32'
  })
  fs.writeFileSync(wasmMeterBinaryFile,meteredWasm) 
  wasmBinaryFile=wasmMeterBinaryFile;
console.log("Execute inject meters code finish.")  
}
//end
...
injectWasmFile();
 ...
 /*
  import code line
   1527 var info = {...}
   2411 if (Module['_main'] && shouldRunNow) Module['callMain'](args);
 */
...
function integrateWasmJS() {
  ...
  var info = {
    ...
    //Add wasm meters by lmh --20181019
    'metering': {
      'usegas': (gas) => {
        gasUsed += gas
        if (gasUsed > limit) {
  ...
  function doNativeWasm(global, env, providedBuffer) {
    ...
    if (!Module['wasmBinary'] &&
      ...
      WebAssembly.instantiateStreaming(fetch(wasmBinaryFile, { credentials: 'same-origin' }), info)
        ...
        .then(receiveInstantiatedSource).catch(function(reason) {
...
Module['wasmTableSize'] = 24;

Module['wasmMaxTableSize'] = 24;
...
Module['callMain'] = function callMain(args) {
  ...
  try {
    ...
    console.log(`***Execute finish.Gas used ${gasUsed * 1e-4}`) 
...
function run(args) {
  ...
  function doRun() {
    ...
    console.log("2.Prepare execute go main function...")   
